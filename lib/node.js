...
Node.prototype.startWeb3Connection = function()
{
    ...
    console.info('http://' + (process.env.RPC_HOST || 'localhost') + ':' + (process.env.RPC_PORT || '8545'));
	web3 = new Web3();
...
Node.prototype.getStats = function(forced)
{
	...
	if (this._web3 && (lastFetchAgo >= UPDATE_INTERVAL || forced === true))
	{
		...
		async.parallel({
			...
			{
                callback(null, web3.net.peerCount);
			},
			mining: function (callback)
			{
                callback(null, web3.eth.mining);
			},
			...
			{
                callback(null, web3.eth.gasPrice);
			},
			syncing: function (callback)
			{
                callback(null,web3.eth.syncing);
			}
		},
